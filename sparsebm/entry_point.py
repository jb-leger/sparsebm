    ...
    for parser in [sbm_parser, lbm_parser]:
        ...
        if parser == sbm_parser:
            param_grp.add_argument(
                "-s",
                "--symetric",
                help="Specify if the adajacency matrix is symetric",
                default=False,
                # type=bool,
            )
        param_grp.add_argument(
...
def process_sbm(args):
    ...
    symetric = str2bool(args["symetric"])
    print("Runing with symetric adjacency matrix : {}".format(symetric))
